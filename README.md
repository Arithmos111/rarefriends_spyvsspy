...
submission pull request needs. Its source links and its screenshot are pinned to a commit, so
run **`npm run submission`** to re-pin them to whatever is being submitted; it prints the
links to check. The screenshot itself is regenerated by **`npm run preview`**, which plays a
real two-agent match in two browsers and captures the frame.

The entry is written to be pasted verbatim into the pull request body as well as committed,
which is why its screenshot is an absolute `raw.githubusercontent.com` URL rather than a
repository-relative path: a relative path renders as a broken image in a pull request.
