...
| The picker finds no Friends | Check the chain: it must be Robinhood mainnet, **4663**. From FriendSDK v0.1.2 the picker names the chain you are on and offers a **Switch to Robinhood** button. The Friend must also be **hardwired**, generation 1 or higher; generation 0 holdings are reported separately. |
| Everything fails with "Failed to fetch" against `rpc.mainnet.chain.robinhood.com` | The Rare Friends RPC is unreachable, which blocks the SDK picker for new players. Nothing here can fix it. Players already in a match keep playing, because the relay tolerates a failed ownership read. Check the endpoint from a second network before assuming it is your host. |
